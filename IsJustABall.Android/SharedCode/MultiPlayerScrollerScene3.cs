...
namespace IsJustABall.Android
{
	...
	//	CCSprite levelcleared;
		...
		{   var bounds = mainWindow.WindowSizeInPixels;
			...
			scoreLabel = new CCLabel("0s","fonts/nasalizationbold.ttf",70);
		...
		void HandleTouchesBegan(System.Collections.Generic.List<CCTouch> touches, CCEvent touchEvent){
			...
			if (hit)
			{
				...
				ResumeGame.RunAction(ZoomTouch);
				CCMoveTo SlideBack = new CCMoveTo (0.5f, new CCPoint (0.5f*bounds.Width,- 0.5f * bounds.Height));
				menuframe.RunAction (SlideBack);
				SlideBack = new CCMoveTo (0.5f, new CCPoint (0.5f*bounds.Width,- 0.3f * bounds.Height));
				ResumeGame.RunAction (SlideBack);
				SlideBack = new CCMoveTo (0.5f, new CCPoint (0.75f*bounds.Width,- 0.3f * bounds.Height));
				Restart.RunAction (SlideBack);
				SlideBack = new CCMoveTo (0.5f, new CCPoint (0.25f*bounds.Width,- 0.3f * bounds.Height));
			...
			if (hit) {
				if (PauseGame == true) {
					...
					CCMoveTo SlideOut = new CCMoveTo (0.5f, new CCPoint (0.5f*bounds.Width,-0.5f * bounds.Height));
		...
		void addMenuOptions (){
			...
			menuframe.PositionY = -0.6f*bounds.Height;
